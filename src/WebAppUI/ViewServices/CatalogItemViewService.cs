...
namespace CnAppForAzureDev.ViewServices
{
    public class CatalogItemViewService : ICatalogItemViewService
    {
        ...
        public async Task<IEnumerable<CatalogItemViewModel>> GetCatalogItemsAsync(ClaimsPrincipal user, string hostname)
        {
            var catalogItems = await _catalogItemRepository.ListAsync(hostname);
